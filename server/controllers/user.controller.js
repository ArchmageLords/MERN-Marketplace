...
const userByID = async (req, res, next, id) => {
  try {
    ...
    return res.status('400').json({
      error: "Не удалось вернуть пользователя"
    })
  }
}
...
const isSeller = (req, res, next) => {
  ...
  if (!isSeller) {
    return res.status('403').json({
      error: "Пользователь не продавец"
    })
  }
  next()
}
      ...
      myStripe.customers.update(req.profile.stripe_customer, {
        ...
        if(err){
          return res.status(400).send({
            error: "Не удалось обновить сведения о платежах"
          })
        }
...
const createCharge = (req, res, next) => {
  if(!req.profile.stripe_seller){
    return res.status('400').json({
      error: "Подключите свою учетную запись Stripe"
    })
  }
