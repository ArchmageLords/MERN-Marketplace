        ...
        <div className={classes.root}>
              <Card className={classes.card}>
                <CardHeader
                  ...
                  subheader={<span>
                    {currentDate < new Date(auction.bidStart) && 'Аукцион не начался'}
                    {currentDate > new Date(auction.bidStart) && currentDate < new Date(auction.bidEnd) && 'Аукцион идёт'}
                    {currentDate > new Date(auction.bidEnd) && 'Аукцион закончился'}
                    </span>}
                />
                <Grid container spacing={6}>
                  ...
                  <Grid item xs={7} sm={7}>
                    ...
                    ? (<>
                        ...
                        { !auth.isAuthenticated() && <Typography>Пожалуйста, <Link to='/signin'>войдите</Link> чтобы поставить вашу ставку.</Typography> }
                        { auth.isAuthenticated() && <Bidding auction={auction} justEnded={justEnded} updateBids={updateBids}/> }
                      </>)
                    : <Typography component="p" variant="h6">{`Аукцион начался ${new Date(auction.bidStart).toLocaleString()}`}</Typography>}
                  </Grid>
