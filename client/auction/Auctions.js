...
export default function Auctions(props){
  ...
  const auctionState = (auction)=>{
    return (
      <span>
          {currentDate < new Date(auction.bidStart) && `Аукцион начался ${new Date(auction.bidStart).toLocaleString()}`}
          {currentDate > new Date(auction.bidStart) && currentDate < new Date(auction.bidEnd) && <>{`Идёт аукцион | ${auction.bids.length} ставок/ки |`} {showTimeLeft(new Date(auction.bidEnd))}</>}
          {currentDate > new Date(auction.bidEnd) && `Аукцион закончился | ${auction.bids.length} ставок/ки `} 
