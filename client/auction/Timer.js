  ...
  const classes = useStyles()
    ...
    return (<div className={classes.subheading}>
        {!timeLeft.timeEnd? <Typography component="p" variant="h6" >
                ...
                {timeLeft.seconds != 0 && `${timeLeft.seconds} s`} осталось до <span className={classes.endTime}>{`(ends at ${new Date(props.endTime).toLocaleString()})`}</span></Typography> : 
            <Typography component="p" variant="h6" >Аукцион закончен</Typography>}
        </div>
    )
}